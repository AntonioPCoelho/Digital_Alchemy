...
// Funcao para calcular a distancia Euclidiana entre dois pixels
double distanciaCor(RGBpixel pixel1, RGBpixel pixel2) {
    int rDiff = pixel1.r - pixel2.r;
    int gDiff = pixel1.g - pixel2.g;
    int bDiff = pixel1.b - pixel2.b;
    return sqrt(0.299 * rDiff * rDiff + 0.587 * gDiff * gDiff + 0.114 * bDiff * bDiff);
}

// Funcao para trocar dois pixels em uma imagem
void trocaPixels(Img *imagem, int index1, int index2) {
    RGBpixel aux = imagem->pixels[index1];
    imagem->pixels[index1] = imagem->pixels[index2];
    imagem->pixels[index2] = aux;
}
...
int main(int argc, char *argv[])
{
    ...
    // Copia a imagem de origem para a imagem resultado
    for (int i=0; i<tam; i++)
    {
    ...
    for (int i = 0; i < pic[DESEJ].width * pic[DESEJ].height; i++) {
    // Acha o pixel mais proximo na imagem de saida
    int indProx = 0;
    double distProx = distanciaCor(pic[DESEJ].pixels[i], pic[SAIDA].pixels[0]);
    for (int j = 1; j < pic[SAIDA].width * pic[SAIDA].height; j += 500) {
        double dist = distanciaCor(pic[DESEJ].pixels[i], pic[SAIDA].pixels[j]);
        if (dist < distProx) {
            indProx = j;
            distProx = dist;
        }
    ...
    // Troca a posicao do pixel encontrado na saida com o pixel correspondente na imagem desejavel
    trocaPixels(&pic[SAIDA], i, indProx);
}
